...
income_ridership_crime_bus<-income_ridership_crime_bus%>%
  ...
  mutate(crime_cat= ntile(crime, 5))%>%
...
income_ridership_crime_bus<-income_ridership_crime_bus%>%
  mutate(composite_cat= case_when(composite <= 7 ~ "Very Low",
                                  ...
                                  composite > 10 & composite <= 14 ~ "Medium",
                                  composite > 14 & composite <= 18 ~ "High",
                                  composite > 18 ~ "Very High"))
```
...
ggplot(income_ridership_crime_bus, aes(fill = composite_cat))+
  geom_sf()+
  scale_fill_manual(values = c("Very Low" = "#0081a7", "Low" = "#00afb9", "Medium" = "#fdfcdc", "High" = "#fed9b7", "Very High"="#f07167"))+
...
```
### Datiled map shows each neighborhood index predictors
