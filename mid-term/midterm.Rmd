...
Even though log prices are more normally distributed, using original prices as our dependent variable make more sense in our cases. First, the log transformed price is not perfectly normally distributed based on the histogram. There are still a long left tail. Second, Original prices directly show actual monetary values, which stakeholders or decision-makers can easily understand and apply. Since our goals of the prediction is directly predicting the housing values, the original price is used as the dependent variable in the model building process.
...
```{r}
model<- lm(price ~ halfbaths + heatedarea + numfirepla + bldggrade + units + age + ac_dummy + heat_dummy + income + crime + transit + canopy + recreation, data = house)
