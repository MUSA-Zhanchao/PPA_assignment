...
```{r}
house<- house %>%
  select(price, bedrooms,yearbuilt,fullbaths, halfbaths, heatedarea, storyheigh, numfirepla, bldggrade,units, heatedfuel, actype)  %>%
...
income<- get_acs(
  ...
  select(estimate, GEOID, geometry)%>%
...
recreation<- st_read("data/recreation.geojson")%>%
  select(X2023, geometry)%>%
  rename(recreation_coverage=X2023)
```
```{r}
...
predictor_vars <- house[, c("bedrooms", "fullbaths", "halfbaths", "heatedarea", "storyheigh", "numfirepla", "bldggrade", "units", "age", "ac_dummy", "heat_dummy", "median_income", "crime_number", "transit_coverage", "canopy", "recreation_coverage")]%>%st_drop_geometry()
...
```
Since there are high correlation between the number of bedrooms and full baths, I will remove the full baths from the model building process. The full baths is removed to avoid the multicollinearity between the variables. 
```{r}
predictor_vars <- house[, c("bedrooms", "halfbaths", "heatedarea", "storyheigh", "numfirepla", "bldggrade", "units", "age", "ac_dummy", "heat_dummy", "median_income", "crime_number", "transit_coverage", "canopy", "recreation_coverage")]%>%st_drop_geometry()
